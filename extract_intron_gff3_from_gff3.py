...
def instert_introns_to_gff3(gff_filename, output_gff3_filename):
    ...
    gff_db = gff_utils.GFFDatabase(
        from_filename=gff_filename, reverse_recs=True)
    t1 = time.time()
    ...
    for gene_id in genes:
        ...
        gene_start = int(
            str(gene_tree[gene_id]['gene']).split(",")[3].strip(" "))
        gene_end = int(
            str(gene_tree[gene_id]['gene']).split(",")[4].strip(" "))
